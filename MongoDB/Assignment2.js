...
//3. use $group to count the number of zip codes in Atlanta
    db.zipcodes.aggregate([{$match:{city:"ATLANTA"}},{$group: { _id: "$city",totalZipCodes:{$count:{}}}}]).pretty()

//4. use $group to find the total population in Atlanta
    db.zipcodes.aggregate([{$match:{city:"ATLANTA"}},{$group: { _id: "$city",totalPopulation:{$sum:"$pop"}}}]).pretty()
...
//3. limit the results to just the first 3 results. What are the top 3 states in population?
    db.zipcodes.aggregate([{$group: { _id: "$state",totalPopulation:{$sum:"$pop"}}},{$sort: {totalPopulation:-1}},{$limit:3}]).pretty()
...
//4. What are the top 3 cities in population in Texas?
    db.zipcodes.aggregate([{$match:{state:"TX"}},{$group: { _id: {state:"$state", city:"$city"},totalPopulation:{$sum:"$pop"}}},{$sort:{totalPopulation:-1}},{$limit:3}]).pretty()

//--------------------------------------------------- Bonus ---------------------------------------------------------------------------------------------------

//1. Write a query to get the average city population for each state
    db.zipcodes.aggregate([{$group:{_id: {city:"$city",state:"$state"}, populationAVG: {$avg:"$pop"}}}]).pretty()

//2. What are the top 3 states in terms of average city population?
    db.zipcodes.aggregate([{$group:{_id: {city:"$city",state:"$state"}, populationSum: {$sum:"$pop"}}},{$group:{ _id: "$_id.state" , AvgCityPopulation:{$avg:"$populationSum"}}},{$sort: {AvgCityPopulation:-1}},{$limit:5}]).pretty()